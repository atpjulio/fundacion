<div class="card">
  <div class="card-block">
    ...
    <div class="form-group">
      <label for="multiple_services" class="control-label">Servicios adicionales</label>
      <div class="row">
        <div class="col-md-10">
        ...
        <div class="col-md-2 text-right">
          <a id="addRowService" href="javascript:void(0)" class="btn btn-oval btn-secondary addRowService">
            Añadir
          </a>
        </div>
      </div>
              ...
              @foreach (old('service_code') as $key => $value)
                <tr>
                  <td><input type="hidden" name="service_code[]" value="{{ $value }}" class="form-control"
                      readonly><input type="text" name="service_show[]" value="{{ old('service_show')[$key] }}"
                      class="form-control" readonly></td>
                  <td><input type="text" name="service_description[]" value="{{ old('service_description')[$key] }}"
                      class="form-control" readonly></td>
                  <td><input type="number" name="service_days[]" value="{{ old('service_days')[$key] }}"
                      class="form-control" min="1"></td>
                  <td><a href="javascript:void(0);" class="removeRowService btn btn-oval btn-danger">Quitar</a></td>
                </tr>
              @endforeach
            @elseif(isset($authorization) and $authorization->services and count($authorization->services) >
              0)
              @foreach ($authorization->services as $key => $authorizationService)
                @if ($key > 0)
                  <tr>
                    <td><input type="hidden" name="service_code[]" value="{{ $authorizationService->service->id }}"
                        class="form-control" readonly><input type="text" name="service_show[]"
                        value="{{ $authorizationService->service->code }}" class="form-control" readonly>
                    </td>
                    <td><input type="text" name="service_description[]"
                        value="{{ $authorizationService->service->name . ' - $' . $authorizationService->service->price }}"
              ...
              @if ($oldVersionService)
                <tr>
                  <td><input type="hidden" name="service_code[]" value="{{ $oldVersionService->id }}"
                      class="form-control" readonly><input type="text" name="service_show[]"
                      value="{{ $oldVersionService->code }}" class="form-control" readonly></td>
                  <td><input type="text" name="service_description[]" value="{{ $oldVersionService->name }}"
                      class="form-control" readonly></td>
                  <td><input type="number" name="service_days[]" value="{{ $authorization->days }}"
                      class="form-control" min="1"></td>
                  <td><a href="javascript:void(0);" class="removeRowService btn btn-oval btn-danger">Quitar</a></td>
                </tr>
              @endif
